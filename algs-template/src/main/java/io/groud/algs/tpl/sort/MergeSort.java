 ...
 * ———————————————————————————————————————————————————————————————————————————————
 * 总体思想：
 * 每次将数组从中间点分为左右两部分，直到长度为 1 时，开始两两合并返回
 * <p>
...
class MergeSortInPlace implements MergeSort {
    ...
    private <T extends Comparable<T>> void mergeSort(T[] a, T[] aux, int left, int right) {
        ...
        if (right + 1 >= left) System.arraycopy(a, left, aux, left, right + 1 - left);

        int i = left, j = mid + 1;
        for (int k = left; k <= right; k++) {
            if (i > mid) a[k] = aux[j++]; // 如果左边已经遍历结束，直接添加右边
            else if (j > right) a[k] = aux[i++]; // 如果右边已经遍历结束，直接添加左边
            else if (less(aux[j], aux[i])) a[k] = aux[j++]; // 否则进行比较大小后先添加小的
            else a[k] = aux[i++];
        }
    }
}
...
class MergeSortNonInPlace implements MergeSort {
    ...
    private <T extends Comparable<T>> T[] sort(T[] a, int left, int right) {
        // 如果二分为 1 个数时直接返回
        if (left == right) return (T[]) new Comparable[]{a[left]};
