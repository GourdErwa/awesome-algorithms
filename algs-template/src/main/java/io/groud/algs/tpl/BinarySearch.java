...
public class BinarySearch {
     ...
     * <p>
     * 初始条件：left = 0, right = length-1 终止：left > right (left = right + 1) 向左查找：right = mid-1 向右查找：left = mid+1
     */
    class TemplateI {
     ...
     * <p>
     * 一种实现二分查找的高级方法。 查找条件需要访问元素的直接右邻居。 使用元素的右邻居来确定是否满足条件，并决定是向左还是向右。 保证查找空间在每一步中至少有 2 个元素。 需要进行后处理。 当你剩下 1
     * 个元素时，循环/递归结束。需要评估剩余元素是否符合条件。
     * <p>
     ...
     * <p>
     * 实现二分查找的另一种方法。 搜索条件需要访问元素的直接左右邻居。 使用元素的邻居来确定它是向右还是向左。 保证查找空间在每个步骤中至少有 3 个元素。 需要进行后处理。 当剩下 2 个元素时，循环 / 递归结束。
     * 需要评估其余元素是否符合条件。
     * <p>
    ...
    /**
     * 技巧、调试方法和注意事项 1. 中间值计算 这里用 (left + right) >>> 1 代替 (left + right) / 2 是非常正确的，首先是因为数组下标肯定不会是负数，另一方面如果 (left +
     * right) 大于 int 最大值时，只有 >>>1 能保证结果正确。
     *
     * 2. 计算中位数 当数组的元素个数是偶数的时候： 使用 mid = left + (right - left) // 2 得到左中位数的索引； 使用 mid = left + (right - left + 1) // 2
     * 得到右中位数的索引。
     *
