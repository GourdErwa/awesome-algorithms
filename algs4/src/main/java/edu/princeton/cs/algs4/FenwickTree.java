/******************************************************************************
 * Compilation: javac FenwickTree.java Execution: java FenwickTree
 *
 * A Fenwick tree.
 *
 ******************************************************************************/
 ...
 * updates</em>. The key point is this: In a fenwick array, each position "responsible" for storing cumulative data of N
 * previous positions (N could be 1) For example: array[40] stores: array[40] + array[39] ... + array[32] (8 positions)
 ...
 * <p>
 * To know the number of items that a given array position 'ind' is responsible for We should extract from 'ind' the
 * portion up to the first significant one of the binary representation of 'ind' for example, given ind == 40 (101000 in
 * binary), according to Fenwick algorithm what We want is to extract 1000(8 in decimal).
 * <p>
 * This means that array[40] has cumulative information of 8 array items. But We still need to know the cumulative data
 * bellow array[40 - 8 = 32] 32 is 100000 in binnary, and the portion up to the least significant one is 32 itself! So
 * array[32] has information of 32 items, and We are done!
 * <p>
 * So cummulative data of array[1...40] = array[40] + array[32] Because 40 has information of items from 40 to 32, and
...
public class FenwickTree {
    ...
    int[] array; // 1-indexed array, In this array We save cumulative information to perform efficient range queries and
                 // updates
    ...
    /**
     * Read the following commands: init n Initializes the array of size n all zeroes set a b c Initializes the array
     * with [a, b, c ...] rsq a b Range Sum Query for the range [a,b] up i v Update the i position of the array with
     * value v. exit
     * <p>
     * The array is 1-indexed Example: set 1 2 3 4 5 6 rsq 1 3 Sum from 1 to 3 = 6 rmq 1 3 Min from 1 to 3 = 1 input up
     * 1 3 [3,2,3,4,5,6]
     *
     * @param args
