/******************************************************************************
 * Compilation: javac StdIn.java Execution: java StdIn (interactive test of basic functionality) Dependencies: none
 *
 ...
 * <ul>
 * <li>those for reading individual tokens from standard input, one at a time, and converting each to a number, string,
 * or boolean
 ...
 * </ul>
 * p> Generally, it is best not to mix functions from the different categories in the same program. p> <b>Getting
 * started.</b> To use this class, you must have {@code StdIn.class} in your Java classpath. If you used our
 ...
 * download <a href = "https://introcs.cs.princeton.edu/java/stdlib/StdIn.java">StdIn.java</a> and put a copy in your
 * working directory. p> <b>Reading tokens from standard input and converting to numbers and strings.</b> You can use
 ...
 * <li>{@link #readBoolean()}
 * </ul>
 * p> The first method returns true if standard input has no more tokens. Each other method skips over any input that is
 * whitespace. Then, it reads the next token and attempts to convert it into a value of the specified type. If it
 * succeeds, it returns that value; otherwise, it throws an {@link InputMismatchException}. p> <em>Whitespace</em>
 * includes spaces, tabs, and newlines; the full definition is inherited from {@link Character#isWhitespace(char)}. A
 * <em>token</em> is a maximal sequence of non-whitespace characters. The precise rules for describing which tokens can
 * be converted to integers and floating-point numbers are inherited from
 ...
 * <li>{@link #readChar()}
 * </ul>
 * p> The first method returns true if standard input has more input (including whitespace). The second method reads and
 ...
 * trailing line separator. p> A <em>line separator</em> is defined to be one of the following strings: {@code \n}
 ...
 * <li>{@link #readAll()}
 * </ul>
 * p> The first three methods read of all of remaining token on standard input and converts the tokens to values of the
 ...
 * <ul>
 * <li>{@code StdIn} is a set of static methods and reads reads input from only standard input. It is suitable for use
 * before a programmer knows about objects. See {@link In} for an object-oriented version that handles input from files,
 * URLs, and sockets.
 * <li>{@code StdIn} uses whitespace as the delimiter pattern that separates tokens. {@link Scanner} supports arbitrary
 * delimiter patterns.
 * <li>{@code StdIn} coerces the character-set encoding to UTF-8, which is the most widely used character encoding for
 * Unicode.
 ...
 * <li>{@code StdIn} has convenient methods for reading a single character; reading in sequences of integers, doubles,
 * or strings; and reading in all of the remaining input.
 * </ul>
 * p> Historical note: {@code StdIn} preceded {@code Scanner}; when {@code Scanner} was introduced, this class was
 * re-implemented to use {@code Scanner}. p> <b>Using standard input.</b> Standard input is a fundamental operating
 * system abstraction on Mac OS X, Windows, and Linux. The methods in {@code StdIn} are <em>blocking</em>, which means
 * that they will wait until you enter input on standard input. If your program has a loop that repeats until standard
 * input is empty, you must signal that the input is finished. To do so, depending on your operating system and IDE, use
 * either {@code <Ctrl-d>} or {@code <Ctrl-z>}, on its own line. If you are redirecting standard input from a file, you
 * will not need to do anything to signal that the input is finished. p> <b>Known bugs.</b> Java's UTF-8 encoding does
 * not recognize the optional <a href = "http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=4508058">byte-order
 * mask</a>. If the input begins with the optional byte-order mask, {@code StdIn} will have an extra character
 * {@code \}{@code uFEFF} at the beginning. p> <b>Reference.</b> For additional documentation, see
 * <a href="https://introcs.cs.princeton.edu/15inout">Section 1.5</a> of <em>Computer Science: An Interdisciplinary
 * Approach</em> by Robert Sedgewick and Kevin Wayne.
 * <p>
 *
...
public final class StdIn {
    ...
    /**
     * Returns true if standard input has a next line. Use this method to know whether the next call to
