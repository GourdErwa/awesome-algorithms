/******************************************************************************
 * Compilation: javac BipartiteMatching.java Execution: java BipartiteMatching V1 V2 E Dependencies: BipartiteX.java
 *
 * Find a maximum cardinality matching (and minimum cardinality vertex cover) in a bipartite graph using the alternating
 * path algorithm.
 *
 ******************************************************************************/
...
/**
 * The {@code BipartiteMatching} class represents a data type for computing a <em>maximum (cardinality) matching</em>
 * and a <em>minimum (cardinality) vertex cover</em> in a bipartite graph. A <em>bipartite graph</em> in a graph whose
 * vertices can be partitioned into two disjoint sets such that every edge has one endpoint in either set. A
 * <em>matching</em> in a graph is a subset of its edges with no common vertices. A <em>maximum matching</em> is a
 * matching with the maximum number of edges. A <em>perfect matching</em> is a matching which matches all vertices in
 * the graph. A <em>vertex cover</em> in a graph is a subset of its vertices such that every edge is incident to at
 * least one vertex. A <em>minimum vertex cover</em> is a vertex cover with the minimum number of vertices. By Konig's
 * theorem, in any biparite graph, the maximum number of edges in matching equals the minimum number of vertices in a
 * vertex cover. The maximum matching problem in <em>nonbipartite</em> graphs is also important, but all known
 * algorithms for this more general problem are substantially more complicated.
 * <p>
 * This implementation uses the <em>alternating-path algorithm</em>. It is equivalent to reducing to the maximum-flow
 * problem and running the augmenting-path algorithm on the resulting flow network, but it does so with less overhead.
 * The constructor takes <em>O</em>((<em>E</em> + <em>V</em>) <em>V</em>) time, where <em>E</em> is the number of edges
 * and <em>V</em> is the number of vertices in the graph. It uses &Theta;(<em>V</em>) extra space (not including the
 * graph).
 * <p>
 * See also {@link HopcroftKarp}, which solves the problem in <em>O</em>(<em>E</em> sqrt(<em>V</em>)) using the
 * Hopcroft-Karp algorithm and <a href =
 * "https://algs4.cs.princeton.edu/65reductions/BipartiteMatchingToMaxflow.java.html">BipartiteMatchingToMaxflow</a>,
 * which solves the problem in <em>O</em>((<em>E</em> + <em>V</em>) <em>V</em>) time via a reduction to maxflow.
 * <p>
...
public class BipartiteMatching {
    ...
    /**
     * Returns true if the graph contains a perfect matching. That is, the number of edges in a maximum matching is
     * equal to one half of the number of vertices in the graph (so that every vertex is matched).
     *
    ...
    /**
     * Unit tests the {@code HopcroftKarp} data type. Takes three command-line arguments {@code V1}, {@code V2}, and
     * {@code E}; creates a random bipartite graph with {@code V1} + {@code V2} vertices and {@code E} edges; computes a
