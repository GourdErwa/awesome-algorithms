/******************************************************************************
 * Compilation: javac AVLTreeST.java Execution: java AVLTreeST < input.txt Dependencies: StdIn.java StdOut.java Data
 ...
 *
 * % java AVLTreeST < tinyST.txt A 8 C 4 E 12 H 5 L 11 M 9 P 10 R 3 S 0 X 7
 *
 ******************************************************************************/
...
/**
 * The {@code AVLTreeST} class represents an ordered symbol table of generic key-value pairs. It supports the usual
 ...
 * <p>
 * This symbol table implementation uses internally an <a href="https://en.wikipedia.org/wiki/AVL_tree"> AVL tree </a>
 * (Georgy Adelson-Velsky and Evgenii Landis' tree) which is a self-balancing BST. In an AVL tree, the heights of the
 ...
 * <p>
 * This implementation requires that the key type implements the {@code Comparable} interface and calls the
 * {@code compareTo()} and method to compare two keys. It does not call either {@code equals()} or {@code hashCode()}.
 * The <em>put</em>, <em>get</em>, <em>contains</em>, <em>delete</em>, <em>minimum</em>, <em>maximum</em>,
 * <em>ceiling</em>, and <em>floor</em> operations each take logarithmic time in the worst case. The <em>size</em>, and
 * <em>is-empty</em> operations take constant time. Construction also takes constant time.
 * <p>
 * For other implementations of the same API, see {@link ST}, {@link BinarySearchST}, {@link SequentialSearchST},
 * {@link BST}, {@link RedBlackBST}, {@link SeparateChainingHashST}, and {@link LinearProbingHashST}.
 *
...
public class AVLTreeST<Key extends Comparable<Key>, Value> {
    ...
    /**
     * Returns the height of the internal AVL tree. It is assumed that the height of an empty tree is -1 and the height
     * of a tree with just one node is 0.
     *
    ...
    /**
     * Inserts the key-value pair in the subtree. It overrides the old value with the new value if the symbol table
     * already contains the specified key and deletes the specified key (and its associated value) from this symbol
    ...
    /**
     * Returns the balance factor of the subtree. The balance factor is defined as the difference in height of the left
     * subtree and right subtree, in this order. Therefore, a subtree with a balance factor of -1, 0 or 1 has the AVL
    ...
    /**
     * Checks if the tree rooted at x is a BST with all keys strictly between min and max (if min or max is null, treat
     * as empty constraint) Credit: Bob Dondero's elegant solution
     *
     * @param x
     *            the subtree
     * @param min
