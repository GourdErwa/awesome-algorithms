/******************************************************************************
 * Compilation: javac SequentialSearchST.java Execution: java SequentialSearchST Dependencies: StdIn.java StdOut.java
 ...
 *
 * % java SequentialSearchST < tinyST.txt L 11 P 10 M 9 X 7 H 5 C 4 R 3 A 8 E 12 S 0
 *
 ******************************************************************************/
...
/**
 * The {@code SequentialSearchST} class represents an (unordered) symbol table of generic key-value pairs. It supports
 * the usual <em>put</em>, <em>get</em>, <em>contains</em>, <em>delete</em>, <em>size</em>, and <em>is-empty</em>
 * methods. It also provides a <em>keys</em> method for iterating over all of the keys. A symbol table implements the
 * <em>associative array</em> abstraction: when associating a value with a key that is already in the symbol table, the
 * convention is to replace the old value with the new value. The class also uses the convention that values cannot be
 * {@code null}. Setting the value associated with a key to {@code null} is equivalent to deleting the key from the
 * symbol table.
 * <p>
 * It relies on the {@code equals()} method to test whether two keys are equal. It does not call either the
 * {@code compareTo()} or {@code hashCode()} method.
 * <p>
 * This implementation uses a <em>singly linked list</em> and <em>sequential search</em>. The <em>put</em> and
 * <em>delete</em> operations take &Theta;(<em>n</em>). The <em>get</em> and <em>contains</em> operations takes
 * &Theta;(<em>n</em>) time in the worst case. The <em>size</em>, and <em>is-empty</em> operations take &Theta;(1) time.
