/******************************************************************************
 * Compilation: javac BinaryStdOut.java Execution: java BinaryStdOut Dependencies: none
 *
 * Write binary data to standard output, either one 1-bit boolean, one 8-bit char, one 32-bit int, one 64-bit double,
 * one 32-bit float, or one 64-bit long at a time.
 *
...
/**
 * <i>Binary standard output</i>. This class provides methods for converting primtive type variables ({@code boolean},
 * {@code byte}, {@code char}, {@code int}, {@code long}, {@code float}, and {@code double}) to sequences of bits and
 * writing them to standard output. Uses big-endian (most-significant byte first).
 * <p>
 * The client must {@code flush()} the output stream when finished writing bits.
 * <p>
 * The client should not intermix calls to {@code BinaryStdOut} with calls to {@code StdOut} or {@code System.out};
