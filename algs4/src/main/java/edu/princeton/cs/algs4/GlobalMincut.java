/******************************************************************************
 * Compilation: javac GlobalMincut.java Execution: java GlobalMincut filename.txt Dependencies: EdgeWeightedGraph.java
 * Edge.java UF.java IndexMaxPQ.java FlowNetwork.java FlowEdge.java FordFulkerson.java In.java StdOut.java Data files:
 * https://algs4.cs.princeton.edu/43mst/tinyEWG.txt https://algs4.cs.princeton.edu/43mst/mediumEWG.txt
 *
 * Computes a minimum cut using Stoer-Wagner's algorithm.
 *
 * % java GlobalMincut tinyEWG.txt Min cut: 5 Min cut weight = 0.9500000000000001
 *
 * % java GlobalMincut mediumEWG.txt Min cut: 25 60 63 96 199 237 Min cut weight = 0.14021
 *
 ******************************************************************************/
...
/**
 * The {@code GlobalMincut} class represents a data type for computing a <em>global minimum cut</em> in a graph with
 * non-negative edge weights. A <em>cut</em> is a partition of the vertices into two nonempty subsets. An edge that has
 * one endpoint in each subset of a cut is a <em>crossing edge</em>. The weight of a cut is the sum of the weights of
 * its crossing edges. A <em>global minimum cut</em> whose weight is no larger than the weight of any other cut.
 * <p>
 * This is an implementation of <em>Stoer-Wagner's algorithm</em>. The constructor takes <em>O</em>(<em>V</em>
 * (<em>V</em> + <em>E</em>) log <em>V</em>) time, where <em>V</em> is the number of vertices and <em>E</em> is the
 * number of edges. The <em>weight</em> and <em>isCut</em> methods take &Theta;(1) time. It uses &Theta;(<em>V</em>)
 * extra space (not including the graph). p> or additional documentation, see
 * <ul>
 * <li>M. Stoer and F. Wagner (1997). A simple min-cut algorithm. <em>Journal of the ACM </em>, 44(4):585-591.
 * </ul>
 *
...
public class GlobalMincut {
    ...
    /**
     * This helper class represents the <em>cut-of-the-phase</em>. The cut-of-the-phase is a <em>minimum s-t-cut</em> in
     * the current graph, where {@code s} and {@code t} are the two vertices added last in the phase.
     */
    ...
    /**
     * Returns {@code true} if the vertex {@code v} is one side of the mincut and {@code false} otherwise. An edge
    ...
    /**
     * Makes a cut for the current edge-weighted graph by partitioning its vertices into two nonempty subsets. The
     * vertices connected to the vertex {@code t} belong to the first subset. Other vertices not connected to {@code t}
    ...
    /**
     * Computes a minimum cut of the edge-weighted graph. Precisely, it computes the lightest of the cuts-of-the-phase
    ...
    /**
     * Returns the cut-of-the-phase. The cut-of-the-phase is a minimum s-t-cut in the current graph, where {@code s} and
     * {@code t} are the two vertices added last in the phase. This algorithm is known in the literature as <em>maximum
