/******************************************************************************
 * Compilation: javac IndexMultiwayMinPQ.java Execution:
 *
...
/**
 * The IndexMultiwayMinPQ class represents an indexed priority queue of generic keys. It supports the usual insert and
 * delete-the-minimum operations, along with delete and change-the-key methods. In order to let the client refer to keys
 * on the priority queue, an integer between 0 and N-1 is associated with each key ; the client uses this integer to
 * specify which key to delete or change. It also supports methods for peeking at the minimum key, testing if the
 * priority queue is empty, and iterating through the keys.
 * <p>
 * This implementation uses a multiway heap along with an array to associate keys with integers in the given range. For
 * simplified notations, logarithm in base d will be referred as log-d The delete-the-minimum, delete, change-key and
 * increase-key operations take time proportional to d*log-d(n) The insert and decrease-key take time proportional to
 * log-d(n) The is-empty, min-index, min-key, size, contains and key-of operations take constant time. Construction
...
public class IndexMultiwayMinPQ<Key> implements Iterable<Integer> {
    ...
    /**
     * Changes the key associated with index i to the given key If the given key is greater, Worst case is O(d*log-d(n))
    ...
    /**
     * Gets an Iterator over the indexes in the priority queue in ascending order The Iterator does not implement the
     * remove() method iterator() : Worst case is O(n) next() : Worst case is O(d*log-d(n)) hasNext() : Worst case is
     * O(1)
     *
