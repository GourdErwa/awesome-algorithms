/******************************************************************************
 * Compilation: javac BinaryOut.java Execution: java BinaryOut Dependencies: none
 *
 * Write binary data to an output stream, either one 1-bit boolean, one 8-bit char, one 32-bit int, one 64-bit double,
 * one 32-bit float, or one 64-bit long at a time. The output stream can be standard output, a file, an OutputStream or
 * a Socket.
 *
...
/**
 * <i>Binary output</i>. This class provides methods for converting primtive type variables ({@code boolean},
 * {@code byte}, {@code char}, {@code int}, {@code long}, {@code float}, and {@code double}) to sequences of bits and
 * writing them to an output stream. The output stream can be standard output, a file, an OutputStream or a Socket. Uses
 * big-endian (most-significant byte first).
 * <p>
 * The client must {@code flush()} the output stream when finished writing bits.
 * <p>
 * The client should not intermix calls to {@code BinaryOut} with calls to {@code Out}; otherwise unexpected behavior
 * will result.
 *
...
public final class BinaryOut {
     ...
     * @throws IllegalArgumentException
     *             if any character in the string is not between 0 and 2<sup>r</sup> - 1
     */
