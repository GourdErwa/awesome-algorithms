/******************************************************************************
 * Compilation: javac BinaryStdIn.java Execution: java BinaryStdIn < input > output Dependencies: none
 *
...
/**
 * <i>Binary standard input</i>. This class provides methods for reading in bits from standard input, either one bit at
 * a time (as a {@code boolean}), 8 bits at a time (as a {@code byte} or {@code char}), 16 bits at a time (as a
 * {@code short}), 32 bits at a time (as an {@code int} or {@code float}), or 64 bits at a time (as a {@code double} or
 * {@code long}).
 * <p>
 * All primitive types are assumed to be represented using their standard Java representations, in big-endian (most
...
public final class BinaryStdIn {
    ...
    /**
     * Reads the next 8 bits from standard input and return as an 8-bit char. Note that {@code char} is a 16-bit type;
     ...
     * @throws NoSuchElementException
     *         if there are fewer than {@code r} bits available on standard input
     ...
     * @throws NoSuchElementException
     *         if standard input is empty or if the number of bits available on standard input is not a multiple of
     *         8 (byte-aligned)
     */
