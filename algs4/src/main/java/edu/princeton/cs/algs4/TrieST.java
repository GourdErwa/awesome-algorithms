/******************************************************************************
 * Compilation: javac TrieST.java Execution: java TrieST < words.txt Dependencies: StdIn.java Data files:
...
/**
 * The {@code TrieST} class represents an symbol table of key-value pairs, with string keys and generic values. It
 * supports the usual <em>put</em>, <em>get</em>, <em>contains</em>, <em>delete</em>, <em>size</em>, and
 * <em>is-empty</em> methods. It also provides character-based methods for finding the string in the symbol table that
 * is the <em>longest prefix</em> of a given prefix, finding all strings in the symbol table that <em>start with</em> a
 * given prefix, and finding all strings in the symbol table that <em>match</em> a given pattern. A symbol table
 ...
 * <p>
 * This implementation uses a 256-way trie. The <em>put</em>, <em>contains</em>, <em>delete</em>, and <em>longest
 * prefix</em> operations take time proportional to the length of the key (in the worst case). Construction takes
 * constant time. The <em>size</em>, and <em>is-empty</em> operations take constant time. Construction takes constant
 * time.
 * <p>
 * For additional documentation, see <a href="https://algs4.cs.princeton.edu/52trie">Section 5.2</a> of <i>Algorithms,
...
public class TrieST<Value> {
    ...
    /**
     * Inserts the key-value pair into the symbol table, overwriting the old value with the new value if the key is
     * already in the symbol table. If the value is {@code null}, this effectively deletes the key from the symbol
     * table.
     *
     * @param key
     *        the key
    ...
    /**
     * Returns all of the keys in the symbol table that match {@code pattern}, where . symbol is treated as a wildcard
     * character.
     *
     * @param pattern
     *        the pattern
    ...
    /**
     * Returns the string in the symbol table that is the longest prefix of {@code query}, or {@code null}, if no such
     * string.
     *
     * @param query
     *        the query string
