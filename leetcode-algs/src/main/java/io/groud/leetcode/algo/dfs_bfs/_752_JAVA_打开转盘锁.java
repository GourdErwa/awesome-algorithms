...
public class _752_JAVA_打开转盘锁 {
    ...
    static class Solution {
        public int openLock(String[] deadends, String target) {
            ...
            if (deals.size() > 7 && deals.containsAll(status(target))) return -1;

            String init = "0000"; // 初始状态
            if (deals.contains(init)) return -1;
            ...
            while (!queue.isEmpty()) {
                for (int i = queue.size() - 1; i >= 0; i--) {
                    ...
                    for (String o : status(curr)) {
                        if (!deals.contains(o) && visited.add(o)) queue.add(o);
                    }
                }
                step++;
            }
        ...
        public int openLock(String[] deadends, String target) {
            ...
            for (String option : options) { // 判断可达状态中最短转动次数
                ...
                for (int i = 0; i < 4; i++) {
                    ...
                    if (num > 5) curStep += 10 - num; // 判断正转倒转
                    else curStep += num;
                }
