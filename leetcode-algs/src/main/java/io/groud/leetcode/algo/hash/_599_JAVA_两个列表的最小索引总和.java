...
/**
 * https://leetcode-cn.com/problems/minimum-index-sum-of-two-lists/ tag：hash
 *
    ...
    public class Solution1 {
        public String[] findRestaurant(String[] list1, String[] list2) {
            ...
            for (int sum = 0; sum < allLength; sum++) {
                for (int i = 0; i <= sum; i++) {
                    if (i < l1Length && sum - i < l2Length && list1[i].equals(list2[sum - i]))
                        res.add(list1[i]);
                }
                if (!res.isEmpty())
                    break;
