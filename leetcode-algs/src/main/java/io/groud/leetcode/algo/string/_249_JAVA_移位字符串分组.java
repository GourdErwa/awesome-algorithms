...
public class _249_JAVA_移位字符串分组 {
     ...
     * <p>
     * 需要考虑问题： 1. 如果出现负数，可能是 z-c = a-d ，他们的差值互为补数，模是 26，因此我们将负数转为正数记录 2. 如果只累加所有差值，可能会因为差值的位置不同导致结果不准确，我们转换为 10 进制的数
     * 比如：[下标 0-1 的差值] * 1 + [下标 1-2 的差值] * 10 + [下标 2-3 的差值] * 100 考虑如果出现 aa,a 这种编码结果会为 0 ，我们在最终结果把字符长度也编码进去
     * <p>
