...
/**
 * https://leetcode-cn.com/problems/clone-graph/ tag：dfs、bfs、dp
 * <p>
 * TODO dp 未完成
 *
...
public class _494_JAVA_目标和 {
    ...
    class Solution {
        ...
        public int findTargetSumWaysHelper(int[] nums, int i, int currVal, int sum) {
            sum += currVal;
            if (++i == nums.length)
                return sum == 0 ? 1 : 0; // 基本情况，数组访问完成后，结果是否为 0

            return findTargetSumWaysHelper(nums, i, +nums[i], sum) + findTargetSumWaysHelper(nums, i, -nums[i], sum);
        }
    }
}
