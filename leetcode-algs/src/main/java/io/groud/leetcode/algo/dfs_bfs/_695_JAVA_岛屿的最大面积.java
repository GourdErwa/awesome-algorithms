...
public class _695_JAVA_岛屿的最大面积 {
    ...
    /**
     * bfs 遍历
     * 以 1 为起点向四个方向进行 bfs 遍历
     * 遍历到的 1 染色为 2 ，染色是为了避免向四周/下次以 1 为起点时的重复遍历
     * 同时面积计数加 1，只要是连同的且不是 2 的说明面积加 1
     * <p>
        ...
        public int maxAreaOfIsland(int[][] grid) {
            ...
            for (int r = 0; r < rows; r++) {
                for (int c = 0; c < cols; c++) {
                    if (grid[r][c] == 1) ans = Math.max(ans, maxAreaOfIslandHelper(grid, rows, cols, r, c));
                }
            }
            return ans;
        }
