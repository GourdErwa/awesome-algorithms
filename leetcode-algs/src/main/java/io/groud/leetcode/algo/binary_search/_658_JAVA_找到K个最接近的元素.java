...
public class _658_JAVA_找到K个最接近的元素 {
    ...
    class Solution {
        public List<Integer> findClosestElements(int[] arr, int k, int x) {
            ...
            if (length <= k) return list; // 如果长度小于等于 k 直接返回
            if (arr[0] > x) return list.subList(0, k); // 如果 x 小于最小值，返回 0-K 个长度
            if (arr[length - 1] < x) return list.subList(length - k, length);
    ...
    class Solution1 {
        public List<Integer> findClosestElements(int[] arr, int k, int x) {
            ...
            while (left < right) {
                ...
                if (diff > diffK) left = mid + 1;
                else right = mid; // 因为可能出现重复，所以不进行减一
            }
