...
public class _21_JAVA_合并两个有序的链表 {
    ...
    /**
     * - 基本情况：l1 或者 l2 有一个到达尾节点说明合并完成，没有达到尾节点的为合并后的链表 - 递推关系： merge(l1,l2) = l1 + merge(l1.next,l2) merge(l1,l2) = l2 +
     * merge(l1,l2.next)
     * <p>
     * - 修改递归函数的参数 -> 根据递推关系更新参数 - 递归调用并返回中间变量 -> 返回合并后的新链表 - 使用递归函数的返回值与当前参数进行计算 -> 当前节点与返回合并后的新链表链接起来完成计算(next 指向返回结果)
     * - 返回最终结果 -> 返回头节点
     */
