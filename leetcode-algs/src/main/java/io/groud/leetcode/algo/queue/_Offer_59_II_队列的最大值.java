...
/**
 * https://leetcode-cn.com/problems/dui-lie-de-zui-da-zhi-lcof/ tag: 队列
 *
...
public class _Offer_59_II_队列的最大值 {
     ...
     * <p>
     * 思路： - 假如 5 入队，那么当前队列中最小值也是 5 ，所以那些小于 5 的就不需要了。 - 假如 5 出队，那么当前队列最大值如果是 5 ，移除。因为使用 > 移除，如果队列中还有 5 ，那么下一个最大值就是 5.
     */
    class MaxQueue {
        ...
        public int pop_front() {
            if (val.isEmpty())
                return -1;
            int r = val.poll();
            if (r == maxVal.peekFirst())
                maxVal.removeFirst(); // 如果当前移除值是最大值进行移除
            return r;
        }
    }
}
